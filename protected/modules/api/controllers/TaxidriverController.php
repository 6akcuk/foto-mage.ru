...
class TaxiDriverController extends ApiController {
  ...
  public function actionGetOrderInfo($city_id) {
    ...
    if ($driver->order) {
      $result = array(
        ...
        'phone' => $driver->order->customer_phone,
        'from' => $driver->order->getFrom(),
        'comment' => $driver->order->from_comment,
        'to' => $driver->order->getTo(),
        'price' => ActiveHtml::price($driver->order->price),
        'xstatus' => $driver->order->status,
        'status' => Yii::t('taxi', $driver->order->status),
        'key' => $driver->order->poll_key,
        'xprice' => $driver->order->price,
      );
    } else {
      $result['order_id'] = 0;
    }
  ...
  public function actionCheckDriver($city_id) {
    ...
    if ($driver) {
      ...
      $result['active_until'] = ActiveHtml::date($driver->activated_until, true, true, false, false, true, $city->timezone);
