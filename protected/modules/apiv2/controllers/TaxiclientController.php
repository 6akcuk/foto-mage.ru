...
class TaxiClientController extends ApiController {
  ...
  public function actionGetStatistic($city_id) {
    ...
    if (Yii::app()->user->getIsGuest()) {
      ...
      if ($order) {
        if (
          ...
          $order_data = $order->getAPIData();
        }
      } else $order_id = 0;
    }

    $this->json = array(
      'drivers_num' => $driversOnlineNum,
      'orders_num' => $queueOrdersNum,
      'order_id' => $order_id,
  ...
  public function actionOrder($city_id) {
    ...
    $order = TaxiOrder::model()->find($criteria);
    if ($order)
      throw new CHttpException(500, 'Вы уже заказали такси');
    ...
    */
    ...
    if ($order->save()) {
      ...
      $result['order_data'] = $order->getAPIData();
    } else {
      $errors = array();
   ...
   * @throws CHttpException
   */
  public function actionListHistory($city_id, $offset = 0) {
    Yii::import('taxi.models.*');
    $result = array();
  ...
  public function actionRepeatOrder($city_id, $order_id) {
    ...
    if ($new_order->save()) {
      $result['success'] = true;
      $result['message'] = 'Заявка успешно размещена';
      $result['order_data'] = $new_order->getAPIData();
    } else {
      $errors = array();
      foreach ($order->getErrors() as $attr => $error) {
        $errors[] = (is_array($error)) ? implode('<br/>', $error) : $error;
