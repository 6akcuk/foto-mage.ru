...
class CategoryController extends Controller {
  ...
  public function actionIndex($offset = 0) {
    ...
    $criteria->limit = Yii::app()->getModule('advert')->advertCategoriesPerPage;
    ...
    $categories = AdvertCategory::model()->findAll($criteria);
    $categoriesNum = AdvertCategory::model()->count($criteria);
  ...
  public function actionAdd() {
    $category = new AdvertCategory('add');
    ...
    if(isset($_POST['name']))
    {
      ...
      if($category->save()) {
        $result['success'] = true;
        $result['message'] = 'Категория объявлений была успешно добавлена';
      }
      else {
        $errors = array();
  ...
  public function actionEdit($id) {
    $category = AdvertCategory::model()->findByPk($id);
    if (!$category)
  ...
  public function actionDelete($id) {
    ...
    echo json_encode(array('message' => 'Категория объявлений успешно удалена'));
    exit;
  }
}