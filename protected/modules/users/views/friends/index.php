...
?>
<div class="tabs">
  <?php echo ActiveHtml::link('Все друзья', '/friends?id='. $user->id, array('class' => 'selected')) ?>
  <?php echo ActiveHtml::link('Друзья онлайн', '/friends?id='. $user->id .'&section=online') ?>
...
<div class="summary_wrap">
  <?php $this->renderPartial('//layouts/pages', array(
    'url' => '/friends?id='. $user->id .'&'. ((sizeof($url_suffix) > 0) ? ''. implode('&', $url_suffix) : ''),
    'offset' => $offset,
    'offsets' => $offsets,
    'delta' => $delta,
  )) ?>
  <div class="fl_r progress"></div>
  <div class="summary">У <?php if ($user->id == Yii::app()->user->getId()): ?>Вас<?php else: ?><?php echo ActiveHtml::lex(2, $user->getDisplayName()) ?><?php endif; ?> <?php echo Yii::t('user', '{n} друг|{n} друга|{n} друзей', $offsets) ?></div>
</div>
